...
function customQuery($querryTable, $querryRow, $customQueryID = false, $querryID, $queryIDSet = '') {
	...
	if ($customQueryID == false) {
		...
		if (strcmp($querryTable, 'issueLink') == 0) {
			$tableIDRow = 'project';
			$queryIDSet = $queryIDSet;
		}
	} else {
...
function getUserData($userDataType, $realName = false) {
	...
	/*if (strcmp($row['userHTTPIP'], $userHTTPIP) == 0 && strcmp($row['userRemoteIP'], $userIP) == 0 && strcmp($row['userSession'], 'active') == 0) {
		if (strcmp($userDataType, 'name') == 0) {//If requesting user Name Data
			if ($realName == true) {
				return customQuery('userdata', 'userName', true, 'userRemoteIP', $userHTTPIP);
			}
			if ($realName == false) {
				return customQuery('userdata', 'userID', true, 'userRemoteIP', $userHTTPIP);
			}
		}
		
		if (strcmp($userDataType, 'projects') == 0) {//If requesting user Project Data
			return explode('|', customQuery('userdata', 'userProjects', true, 'userHTTPIP', $userHTTPIP));
		}
		
		if (strcmp($userDataType, 'projectsCount') == 0) {//If requesting user Project Count
			return count(explode('|', customQuery('userdata', 'userProjects', true, 'userHTTPIP', $userHTTPIP)));
		}
	}*/
	if (strcmp($userDataType, 'name') == 0) {
		if ($realName == true) {
			if(strcmp(customQuery('userdata', 'userSession', true, 'userRemoteIP', $userHTTPIP), 'active') == 0){
				return customQuery('userdata', 'userName', true, 'userRemoteIP', $userHTTPIP);	
			}
		}
		if ($realName == false) {
