...
";
		...
		<div id="mTurnTable">
			<div class="mTurnTableElement">
				<div class="mTurnTableElementTitle">
					Element Title 1
				</div>
				<div class="mTurnTableElementTag">
					Element Tag 1
				</div>
			</div>
			<div class="mTurnTableElement">
		...
		<script>
			...
			function init() {
				//Cutoff for turn table size is 900px
				$('#mTurnTable').hide();
				if($(window).width() <= 900){
					$('#turnTable').hide();
					$('#mTurnTable').show();
					$('#aboutMe').css({
						'margin-left' : ($(window).width() - 162) / 10
					});
					$('#mTurnTable').width($(window).width());
				}
				//************************\\
				$('#mobileSitePopup').hide();
				$('#mobileSitePopup').width($(window).width());
				//Commented out b/c of responsive design
